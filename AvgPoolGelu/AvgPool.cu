...
__global__ void avgPoolKernel(float* c, const float* a, const int* b)
{
...
// put gelu here // 
    ...
    c[i + dj * j] = sum; // 0.5f * sum * (1 + tanhf(sqrt2Divpi * (sum + 0.044715 * (sum * sum * sum))));
...
cudaError_t avgPoolWithCuda(float* c, float* a, int* b, unsigned int inputSize, unsigned int output_size_i, unsigned int output_size_j)
{
    float* dev_a = 0;
    int* dev_b = 0;
    float* dev_c = 0;
    ...
    avgPoolKernel << <dim3(output_size_i / 4, output_size_j / 4), dim3(4, 4) >> > (dev_c, dev_a, dev_b);
